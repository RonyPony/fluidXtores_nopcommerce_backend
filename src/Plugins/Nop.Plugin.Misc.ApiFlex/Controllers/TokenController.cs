...
using Nop.Plugin.Misc.ApiFlex.Configuration;
using Nop.Plugin.Misc.ApiFlex.Domain;
using Nop.Plugin.Misc.ApiFlex.Infrastructure;
using Nop.Plugin.Misc.ApiFlex.Models.Authentication;
...
namespace Nop.Plugin.Misc.ApiFlex.Controllers
{
    ...
    public class TokenController : Controller
    {
        ...
        private async Task<TokenResponse> GenerateTokenAsync(Customer customer)
        {
            ...
            var customerRolesCollection = await _customerService.GetCustomerRolesAsync(customer, false);
            foreach (var customerRole in customerRolesCollection)
            {
                claims.Add(new Claim(ClaimTypes.Role, customerRole.Name));
            }

            var signingCredentials = new SigningCredentials(new SymmetricSecurityKey(Encoding.UTF8.GetBytes(_apiSettings.SecurityKey)),
                                                            SecurityAlgorithms.HmacSha256);

            var token = new JwtSecurityToken(new JwtHeader(signingCredentials), new JwtPayload(claims));
            var accessToken = new JwtSecurityTokenHandler().WriteToken(token);

            TokenResponse tk = new TokenResponse(accessToken, expiresInSeconds);
            Task<TokenResponse> ttr = Task.Run(() => { return tk; });
            //return Task.Delay(10000).ContinueWith(t => tk);
            return tk;
        }
    }
}
