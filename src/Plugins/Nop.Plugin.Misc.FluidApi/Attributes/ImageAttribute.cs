...
namespace Nop.Plugin.Misc.FluidApi.Attributes
{
    public class ImageValidationAttribute : BaseValidationAttribute
    {
        ...
        public override void Validate(object instance)
        {
            ...
            if (imageSrcSet || imageAttachmentSet)
            {
                ...
                if (_errors.Count == 0)
                {
                    ...
                    ValidatePictureBiteArrayAsync(imageBytes, mimeType,imageDto.SeoFilename);
                }
        ...
        private async Task ValidatePictureBiteArrayAsync(byte[] imageBytes, string mimeType,string filename)
        {
            if (imageBytes != null)
            {
                try
